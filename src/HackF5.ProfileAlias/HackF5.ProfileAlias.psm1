...
function Get-ProfileAliasJson {
    if (-not (Test-Path -Path $path)) {
        return @()
    }

    return Get-Content (Get-ProfileAliasJsonPath) | ConvertFrom-Json
}

function Set-ProfileAliasJson {
    param (
        [Parameter(Mandatory=$true)] [Object[]] $Aliases
    )
...
function Import-ProfileAliasModule {
    $module = New-Object System.Text.StringBuilder("# auto-generated by HackF5.ProfileAlias")

    foreach ($alias in Get-ProfileAliasJasonAliases) 
    {   
        $null = $module.AppendLine();
        $null = $module.AppendLine();

        if ($alias.Extended) {
            $functionName = "Publish-ProfileAliasGenerated_$($alias.name)"
            $null = $module.AppendLine("function $functionName { $($alias.command) `$args }");
            $null = $module.AppendLine("Set-Alias -Name $($alias.name) -Value $functionName -Scope Global -Option ReadOnly");
        }
        else {
            $null = $module.AppendLine("Set-Alias -Name $($alias.name) -Value $($alias.command) -Scope Global -Option ReadOnly");
        }
    }

    $modulePath = Get-ProfileAliasModulePath
    $null = Set-Content -Path $modulePath -Value $module.ToString() -Force -Confirm:$false
    $null = Import-Module $modulePath -Global -Force -Confirm:$false
}
...
function Set-ProfileAlias {
    ...
    if (($null -ne $systemAlias) -and (-not $Force))
    {
        ...
        Write-Error $systemAlias
        return
    }

    $aliases = @(Get-ProfileAliasJasonAliases | Where-Object {$_.name -ne $Name})

    $alias = [PSCustomObject]@{ name = $Name; command = $Command; extended = $Extended.IsPresent }
    $aliases += $alias

    if ($PSCmdlet.ShouldProcess($jsonPath , "Set profile alias $Name in ")) {
        Set-ProfileAliasJson $aliases
        $null = Import-ProfileAliasModule
...
function Remove-ProfileAlias {
    ...
    if ($PSCmdlet.ShouldProcess($jsonPath , "Remove profile alias $Name from ")) {
        Set-ProfileAliasJson $aliases
        $null = Remove-Alias -Name $Name -Force -Confirm:$false
        $null = Import-ProfileAliasModule
